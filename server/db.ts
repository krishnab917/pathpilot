...
export async function listVerifiedOpportunities(userId: string, filters: { category?: OpportunityCategory; alignedOnly?: boolean } = {}) { const db = client(); let query = db.from("opportunities").select("*, opportunity_sources(name), student_opportunity_states(status)").eq("status", "active").order("verified_at", { ascending: false }).limit(250); if (filters.category) query = query.eq("category", filters.category); const { data, error } = await query; check(error); const matches = filters.alignedOnly ? await getCareerMatches(userId) : []; const targetDomains = new Set(matches.flatMap(match => careerDomainsFor(match.career.name))); return list(data as any[]).map(opportunity).map(item => ({ ...item, alignedCareers: matches.filter(match => careerDomainsFor(match.career.name).some(domain => item.careerDomains.includes(domain))).map(match => match.career.name) })).filter(item => item.savedStatus !== "dismissed").filter(item => !filters.alignedOnly || !targetDomains.size || item.alignedCareers.length > 0); }
...
export async function refreshCuratedOpportunityCatalog() {
  const [nasa, drafts] = await Promise.all([refreshNasaSpaceAppsOpportunity(), fetchCuratedOpportunityDrafts()]);
  const admin = serviceClient();
  ...
  return { nasa, imported: rows.length, categoryCounts: rows.reduce<Record<string, number>>((counts, row) => ({ ...counts, [row.category]: (counts[row.category] ?? 0) + 1 }), {}) };
}
...
export async function createAdaptiveSimulation(userId: string, career: string) {
  ...
  const { data, error } = await client().from("simulations").insert({ user_id: userId, career, title: graph.title, scenarios: [], user_choices: [], engine_version: "adaptive-v2", scenario_graph_id: graph.id, current_node_id: state.currentNodeId, node_history: [], decision_history: [], simulation_state: state, behavioral_evidence: [], behavioral_events: [] }).select().single();
