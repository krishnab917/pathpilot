...
- [ ] Performance implementation checkpoint (approval required): implement only evidence-backed boot, route-loading, query-cache, database-projection, and progressive-content optimizations while preserving the current PathPilot navigation and Supabase data authority.
- [ ] Performance AI checkpoint (approval required): improve version-aware AI reuse, bounded context retrieval, duplicate prevention, feedback timing, and appropriate model handling without weakening the checkpoint-65 rate limits or exposing private data.
- [ ] Performance cache-correctness checkpoint (approval required): add cache-isolation, targeted-invalidation, user-switch/logout, request-cancellation, search-debounce, background-deduplication, and AI-fallback regressions for changed behavior.
- [ ] Performance validation checkpoint (approval required): compare before/after measurements; rerun full security, RLS, Mentor-isolation, rate-limit, TypeScript, and production-build validation; publish evidence.
